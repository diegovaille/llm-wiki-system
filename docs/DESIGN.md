...
6. Inferred edges: backlinks and `sources:` overlap: `min(source * 0.3 + inferred_edge_weight, source)`, distinct class. A page keeps the score of the first edge that reaches it: curated edges are applied before inferred ones, in index order. A graph row never outranks the direct match it came from; it can outrank weaker direct matches, including other neighbors of the same source that matched a few terms - that is the designed reach of the edge weights (three sibling caps were measured and withdrawn in 0.5.2-0.5.4; see CHANGELOG). On a tie the direct match wins. Direct matches keep their own scores.
