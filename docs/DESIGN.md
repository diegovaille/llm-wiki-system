...
6. Inferred edges: backlinks and `sources:` overlap: `min(source * 0.3 + inferred_edge_weight, source)`, distinct class. Each of those is further capped at the lowest score among that source's other neighbors that matched at least two query terms; a page reached from several sources keeps the best of its per-source capped scores. So a graph row never outranks the direct match it came from, nor a two-term sibling of that source; a one-term sibling neither caps nor competes. On a tie the direct match wins. Direct matches keep their own scores.
